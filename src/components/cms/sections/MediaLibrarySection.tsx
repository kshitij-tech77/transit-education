...
export function MediaLibrarySection({ data, onToast, onUpload, onDeleteMedia }: MediaLibrarySectionProps) {
  ...
  const allFiles = (Object.values(data.media ?? {}).flat() as MediaFile[]).sort(
  ...
  return (
    <>
        ...
        <div className="flex justify-between items-center bg-white p-5 rounded-xl border border-gray-200">
          <div className="flex gap-3 flex-1 max-w-md">
            <div className="relative flex-1">
              ...
              <input type="text" placeholder="Search media..." className="w-full pl-10 pr-4 py-2 border border-gray-200 rounded-lg text-[13px] outline-none" />
            </div>
          </div>
          <div className="flex gap-3">
            <CmsButton variant="secondary"><Filter size={14} /> Filter</CmsButton>
      ...
      {previewFile && (
        <div onClick={() => setPreviewFile(null)} className="fixed inset-0 z-[300] bg-black/80 flex items-center justify-center p-6">
          <div onClick={e => e.stopPropagation()} className="bg-white rounded-2xl overflow-hidden shadow-2xl max-w-2xl w-full flex flex-col">
            <div className="relative">
              <img src={previewFile.path} alt={previewFile.name} className="w-full max-h-[60vh] object-contain bg-gray-50" />
              <button
            ...
            <div className="p-5 space-y-3">
              <p className="text-[13px] font-bold text-black truncate">{previewFile.name}</p>
              <p className="text-[11px] text-gray-400">{previewFile.size}</p>
              <div className="flex items-center gap-2 bg-gray-50 rounded-lg px-3 py-2">
                <code className="flex-1 text-[11px] text-gray-600 truncate">{previewFile.path}</code>
                <button
                  onClick={() => { void navigator.clipboard.writeText(previewFile.path); onToast("URL copied!"); }}
