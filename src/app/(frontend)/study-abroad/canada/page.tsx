...
export default async function CanadaPage() {
  const [countryRes, faqsRes] = await Promise.all([
    ...
    supabase
      .from('faqs')
      .select('*')
      .eq('page_path', 'study-abroad/canada')
      .eq('status', 'published')
  ...
  // Fallback to Global FAQs if none for Canada
  ...
  return (
    <main>
      <DestinationHero 
        title={country.hero_title || "Study in Canada"}
        subtitle="Study Abroad"
        description={country.why_study || "Canada offers an overabundance of opportunities for international students. It is home to some of the top 100 universities in the world."}
          ...
          <div className="grid lg:grid-cols-2 gap-16">
            ...
            <div className="bg-white p-10 rounded-[2rem] border border-gray-100 shadow-xl">
              ...
              <p className="text-gray-600 mb-8 leading-relaxed">
                {country.intakes || "Canada normally has three intakes in a year: January, May & September. We suggest students start their application at least 4 months before."}
              </p>
            ...
            <div className="grid md:grid-cols-2 gap-8">
              <div className="p-8 rounded-3xl bg-brand/5 border border-brand/10">
                ...
                <ul className="space-y-4">
                  {(country.entry_requirements.ug || ["At least 3.0 GPA in High School Certificate.", "IELTS 6.5 with each band 6.0 (Diploma: 6.0 with each band 5.5)."]).map((req: string, i: number) => (
              ...
              <div className="p-8 rounded-3xl bg-black/5 border border-black/10">
                ...
                <ul className="space-y-4">
                  {(country.entry_requirements.pg || ["At least first division in Bachelors Degree.", "IELTS 6.5 overall with each band 6.0."]).map((req: string, i: number) => (
                    <li key={i} className="flex gap-3 text-gray-700">
