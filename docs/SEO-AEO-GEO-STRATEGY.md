...
**After:** `src/app/sitemap.ts` — a native Next.js Metadata Route, `revalidate = 300` — queries Supabase directly for LIVE countries, all branches, and published blog posts, and builds their URLs (including all four `visa|scholarships|cost|universities` sub-guides per country) alongside a differentiated static-route list (homepage priority 1.0/daily, country hubs 0.9/weekly, legal pages 0.2/yearly, etc.). This means **the sitemap is always in sync with what's actually LIVE in the CMS** — an editor publishing a new country or branch doesn't require a code change or a redeploy to appear in the sitemap. Verified live: **43 → 72 URLs**, with the 4 previously-missing branch pages and all previously-missing country sub-guides now present (see the implementation checklist for the exact verification commands run).

**Two corrections made during pre-PR review, before this branch was pushed** (documented here rather than silently folded in, since both were genuine bugs the first draft of `sitemap.ts` introduced):
1. The first draft only pulled country hub/sub-guide URLs from the `countries` table, which correctly covers the 5 CMS-backed static country pages (Canada/USA/UK/Germany/Australia) but **initially omitted the 4 fully hand-authored static pages (Italy/South Korea/Ireland/New Zealand)**, since they have no matching `countries` row at all in the current database — they'd have silently dropped out of the sitemap entirely (a regression versus the old `next-sitemap` output, which found them via static-file crawling regardless of DB state). Fixed by explicitly listing those 4 as always-present URLs, and excluding their ids from the dynamic query's output so the same URL can never appear twice. Their sub-guide URLs (`/study-abroad/italy/visa` etc.) are correctly **not** included, since those genuinely 404 (the DB has no LIVE row to serve them from) — verified directly.
2. The first draft also included a `/study-abroad` "hub" URL at priority 0.9 that **does not exist as a real page** (there's a `study-abroad/[slug]/page.tsx` and `study-abroad/[slug]/[subpage]/page.tsx`, but no `study-abroad/page.tsx` — visiting `/study-abroad` genuinely 404s). That would have submitted a 404 URL to Search Console. Removed.

After both fixes, every one of the 72 sitemap URLs was individually curl-verified to return `200`, have no `noindex` meta, and canonicalize to itself — see the implementation checklist for the exact commands.
...
- **Google Search Console** (verify domain if not already; not inspectable from code) — monitor sitemap coverage before/after this PR specifically (43 → 72 submitted URLs is directly verifiable there within days of deploy), track impressions/clicks for the newly-indexable location and country-subpage URLs.
...
- **Did not** fix a pre-existing, newly-surfaced title-duplication bug on the DB-driven `/study-abroad/france` page (`<title>Study in France | Transit Education | Transit Education</title>` — the site name template appears to be applied twice, likely because the CMS's `meta_title` field for this country already includes it). This bug predates this PR (this repo never touched `study-abroad/[slug]/page.tsx`'s metadata logic) and was simply invisible before because the page had no sitemap entry to be found on. Fixing it is a small, well-scoped follow-up but is out of this PR's audited scope — flagged in the implementation checklist instead.
