...
| Change | Files | Why |
|---|---|---|
| Native dynamic sitemap querying Supabase directly (countries, branches, blog posts) + explicit inclusion of the 4 hand-authored country pages that have no DB row | `src/app/sitemap.ts` (new) | Old `next-sitemap` build-time crawl only found static HTML; missed all 4 branch pages and ~36 country sub-guide pages. Verified live: **43 → 72 URLs**, every one individually confirmed `200` with no `noindex` and a self-matching canonical. |
...
- `npm run start` on a built production server, then verified with `curl` against the live server:
  - `/robots.txt` renders with the expected disallow rules (`/cms`, `/admin`, `/portal`, `/api/`, `/auth/` — nothing else) and `Sitemap:`/`Host:` pointing at `transiteducation.com.np`.
  - `/sitemap.xml` renders exactly 72 `<loc>` entries (up from 43), all on the correct `https://transiteducation.com.np` host, zero duplicates, zero `/cms`/`/admin`/`/portal`/`/api`/`/auth` URLs. Includes all 4 `/locations/{kathmandu,itahari,damak,damauli}`, all 9 `/study-abroad/{country}` hub pages (5 CMS-backed + 4 hand-authored), the DB-added `/study-abroad/france`, and all `visa/scholarships/cost/universities` sub-guides for the 6 countries that actually have LIVE `countries` rows (correctly excluding sub-guides for the 4 hand-authored countries, which genuinely 404 — verified directly).
  - **Every one of the 72 sitemap URLs was individually fetched and confirmed**: `200` status, no `noindex` in its `<meta name="robots">`, and (where a canonical tag is set) a canonical that matches its own sitemap URL exactly.
  ...
  - Homepage JSON-LD now includes the enriched `address`/`sameAs` Organization fields, using only data already published elsewhere on the site.
  - `/locations/kathmandu` now includes `BreadcrumbList` JSON-LD.
  - Confirmed `FAQPage` schema does **not** render on pages with no FAQ rows yet (e.g. `/services/admission-counselling` today) — the plumbing is purely conditional, nothing fabricated.
  - Confirmed the 4 hand-authored country pages (`/study-abroad/{italy,south-korea,ireland,new-zealand}`) are byte-for-byte unchanged by this PR (`git diff` against the pre-PR commit shows zero diff on those 4 files) and still render `200` / `index, follow` with their original titles.
...
- **The 4 hand-authored static country pages (Italy, South Korea, Ireland, New Zealand)** bypass the CMS entirely — an editor's changes to those countries in the admin dashboard have zero effect on the live page, and they have no matching `countries` row in the database at all today (confirmed during pre-PR review — see the strategy doc §3.3 for how that was caught and fixed for sitemap purposes specifically). They **are** correctly included in the sitemap and remain fully indexable/unchanged by this PR. What's still open: the fix for CMS-editability (switching their metadata to the CMS-aware `getCountryMetadata()` helper, or migrating them to `CountryDestinationPage`) requires **first creating and populating matching `countries` rows** with equivalent content — doing this blind risks silently `noindex`-ing four fully live pages that currently rank. Needs a database/content decision: migrate to CMS-driven, or keep them intentionally hand-authored and just document why.
...
- **Newly-surfaced pre-existing bug: `/study-abroad/france` has a duplicated title** (`Study in France | Transit Education | Transit Education`) — this predates this PR and was invisible before because the page had no sitemap entry. Likely the CMS's `meta_title` field for France already includes the site name, which then gets the `%s | Transit Education` template applied on top. Not fixed here (out of this PR's audited scope) — worth a quick follow-up: either strip the suffix from the stored `meta_title` value via the CMS, or adjust `study-abroad/[slug]/page.tsx`'s `generateMetadata` to not double-apply it.
